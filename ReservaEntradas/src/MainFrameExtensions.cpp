...
void bubbleSortManual(Reserva** arr, int n, bool (*criterioMayor)(Reserva*, Reserva*)) {
    ...
    for (int i = 0; i < n - 1; i++) {
        ...
        // [OPTIMIZACION]: Si en una pasada no hubo cambios, ya esta ordenado. Salir.
...
void MainFrame::UpdateListFromArray(Reserva** arr, int n) {
    ...
    // [OPTIMIZACION]: Congelar la UI mientras se actualiza la lista masivamente
    // Esto previene que la pantalla se "trabe" o parpadee durante la carga.
    listView->Freeze(); 
    ...
    // Descongelar la UI para mostrar los cambios de golpe
    listView->Thaw();
}
